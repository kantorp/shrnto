  ...
  return (
    <main className="flex min-h-screen flex-col bg-bg">
          ...
          {LANGS.map((l, i) => (
            <span key={l.value} className="flex items-center gap-1">
              <button
                ...
                className={prefs.language === l.value ? "text-brand" : "text-ink-4 hover:text-ink-2"}
              >
                {l.label}
              </button>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="mt-8 font-display text-2xl font-medium text-ink">{t.login.tagline}</h1>
        <p className="mt-2 font-sans text-[15px] text-ink-3">{t.login.subtitle}</p>
